...
public class DownloadService extends Service {
	...
	public final static String DEFAULT_SAVE_FILE_PATH = FileUtil.getSDPath()
			+ "/" + MyConstants.DOWNLOAD;
		...
		public void handleMessage(Message msg) {
			...
			case 1:
				...
				if (rate < 100) {
					...
					Intent intent = new Intent(mContext, WLMainActivity.class);
					// 告知已完成
					...
					mNotification = builder.setContentIntent(contentIntent).setContentTitle(getResources()
							.getString(R.string.download_over)).setContentText(getResources()
									.getString(R.string.file_download_over)).build();
//					mNotification.setLatestEventInfo(mContext, getResources()
//							.getString(R.string.download_over), getResources()
//							.getString(R.string.file_download_over),
//							contentIntent);
	...
	public IBinder onBind(Intent intent) {
		downloadUrl = intent.getStringExtra(BUNDLE_KEY_DOWNLOAD_URL).toString()
				.trim();
		saveFileName = saveFileName
				+ getSaveFileName(downloadUrl).toString().trim();
