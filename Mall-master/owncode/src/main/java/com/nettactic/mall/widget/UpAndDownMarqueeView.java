...
/**
 * 跑马灯效果<向上方向>的  MarqueeView
 * Created by heshicaihao on 2017/06/16.
 */
public class UpAndDownMarqueeView extends FrameLayout {
    private ArrayList<String> marqueeData;
    private int internal = 1000;//默认滚动时间间隔
    ...
    public UpAndDownMarqueeView(Context context) {
        this(context, null);
    }

    public UpAndDownMarqueeView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public UpAndDownMarqueeView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        //init default attrs
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.UpAndDownMarqueeStyle, defStyleAttr, 0);
        internal = typedArray.getInteger(R.styleable.UpAndDownMarqueeStyle_marqueeInterval, internal);
        textSize = typedArray.getDimensionPixelSize(R.styleable.UpAndDownMarqueeStyle_marqueeTextSize, textSize);
        textColor = typedArray.getColor(R.styleable.UpAndDownMarqueeStyle_marqueeTextColor, textColor);
        int gravity = typedArray.getInt(R.styleable.UpAndDownMarqueeStyle_marqueeTextGravity, 0);
        switch (gravity) {
            case 0:
